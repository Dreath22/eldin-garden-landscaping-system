import { switchTab, renderPagination } from './utils/utils.js';
      ...
      async function fetchUsers(page = state.currentPage) {
        ...
        try {
          ...
          renderPagination(data.summary.roleFilterValue, state, state.limit);
        } catch (error) {
    ...
    `,
      ...
      function showViewUserModal(user) {
        let doc = `<div class="modal modal-large">
...
window.document.querySelector("[data-action='showadd-user']").onclick = () => ocument.getElementById("addUserModal").style.display = "flex";
